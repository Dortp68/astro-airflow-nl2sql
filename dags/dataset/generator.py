...
class InstructionDatasetGenerator(BaseGenerator):
    ...
    def generate_sql(self, complexity: int, batch_size: int) -> SQLSample:
    ...
    def generate_nl(self, query: str, num_questions: int) -> NLSample:
    ...
    def generate_dataset(self, num_samples: int):
        ...
        for complexity in COMPLEXITY_LEVELS:
            ...
            validated_sql = self.validate_sql(queries=sql_queries.queries)

            for sql in validated_sql:
                nl_queries = self.generate_nl(query=sql, num_questions=1)
                for nl in nl_queries.queries:
                    self.dataset.instructions["queries"].append(nl)
                    self.dataset.answers["queries"].append(sql)
...
Return only the SQL query, no explanations."""
    ...
    def generate_dataset(self, num_samples: int):
        if self.instruction_dataset is None:
            raise ValueError(
                "Instruction dataset is required for PreferenceDatasetGenerator"
            )
        num_samples = min(
            num_samples, len(self.instruction_dataset.instructions["queries"])
        )

        for i in range(num_samples):
            instr = self.instruction_dataset.instructions["queries"][i]
            chosen = self.instruction_dataset.answers["queries"][i]
            nl_question = instr["queries"][0] if isinstance(instr, dict) else str(instr)
            rejected = self.generate_rejected_sql(nl_question)
            self.dataset.instructions["queries"].append(instr)
            self.dataset.answers["queries"].append(chosen)
